...
from .database import get_latest_stats, get_all_stats, add_entry, create_tables, clean_old
...
def _get_reader(conf: Config) -> Reader:
    if conf.reader_type == 'MOCK':
        ...
        return NovaPmReader(usb_path=conf.usb_path,
...
async def read_from_device() -> None:
    ...
    try:
        ...
        await add_entry(dbconn=database, event_time=event_time, epa_aqi_pm25=epa_aqi_pm25, raw_pm25=result.pmtwofive, raw_pm10=result.pmten)
...
async def add_new_entry() -> AqiRead:
    ...
    await add_entry(dbconn=database, event_time=datetime.now(), epa_aqi_pm25=epa_aqi_pm25, raw_pm25=data.pmtwofive, raw_pm10=data.pmten)
    return data
