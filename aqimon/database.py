...
async def get_latest_stats(dbconn: databases.Database):
    result = await dbconn.fetch_one(
        "SELECT * FROM aqi_log ORDER BY event_time DESC LIMIT 1"
    )
    if result:
...
async def add_entry(
    dbconn: databases.Database, event_time, epa_aqi_pm25, raw_pm25, raw_pm10
):
...
async def create_tables(dbconn: databases.Database):
    ...
    await dbconn.execute(
        """CREATE INDEX IF NOT EXISTS aqi_eventtime ON aqi_log (event_time)"""
    )
